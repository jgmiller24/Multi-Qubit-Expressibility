...
This experiment extends training to 1000 epochs to determine whether performance
continues to improve or plateaus, and to establish the true performance ceiling
of the 2-qubit configuration.

This run is designed to:
- Match Experiment 0 training duration for direct comparability
- Identify convergence behavior (plateau vs continued improvement)
- Measure the train/test gap at full training
...
Hypothesis:
If the model is approaching its representational limit, then extending training
to 1000 epochs will yield minimal additional improvement and clearly reveal a
performance plateau. If performance continues to improve significantly, then
the model may still be under-trained rather than capacity-limited.
"""
