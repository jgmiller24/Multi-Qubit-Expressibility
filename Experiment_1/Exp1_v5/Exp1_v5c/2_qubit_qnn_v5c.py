"""
Experiment 1 (v5c): 2-qubit, 4-class hybrid QNN with revised observables – EPOCHS = 600

Objective:
Evaluate whether improved observable design continues to provide performance
advantages when fully trained, and determine if observable-based improvements
surpass or converge with increased circuit depth at convergence.

Rationale:
Previous experiments showed:
- v4/v4b/v4c: Increased circuit depth improves performance when sufficiently trained
- v5/v5b: Improved observables provide more efficient performance gains

However, both approaches appeared to converge toward a similar performance range.
This experiment extends training to 600 epochs to determine whether observable
design leads to a higher performance ceiling or simply faster convergence.

Hypothesis:
If observable design provides fundamentally better feature extraction, then the
revised observable set should outperform depth-based approaches at convergence.
Otherwise, both configurations will converge to a similar accuracy range,
indicating a shared representational limitation.

Control Variables:
- Same 2-qubit architecture (fixed Hilbert space)
- Same dataset, preprocessing, and training pipeline
- Same optimization settings as prior experiments
- Circuit structure identical to v3 (no added depth)
...
plt.axis("off")
plt.savefig("sample_inputs_Exp1_v5c.png", dpi=200, bbox_inches="tight")
plt.close()
...
plt.title("Experiment 1 (v5c) Confusion Matrix")
plt.tight_layout()
plt.savefig("experiment1v5c_confusion_matrix.png", dpi=200)
plt.close()
...
plt.tight_layout()
plt.savefig("experiment1v5c_metrics.png", dpi=200)
plt.close()
