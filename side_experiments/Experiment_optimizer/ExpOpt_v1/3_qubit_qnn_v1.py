"""
Experiment Optimizer (v1): 3-Qubit QNN Optimization Study - Reduced Regularization (Adadelta) - EPOCHS = 200

Objective:
Evaluate whether excessive regularization (high weight decay) is limiting learning and slowing convergence in the 3-qubit QNN.

Configuration:
- Model: Exp2_v3 (3-qubit, 8-class classification)
- Circuit: 2-layer variational circuit with entanglement (unchanged)
- Observables: Same 8 observable set (unchanged)
- Sample size: 4000
- Epochs: 200
- Optimizer:
    - Adadelta
    - lr = 0.001
    - weight_decay = 1e-4 (reduced from 0.8)

Key Questions:
- Does reducing weight decay improve early learning (0-200 epochs)?
- Does convergence speed increase compared to baseline?
- Are previously underperforming classes (collapse cases) improved?
- Is training stability affected (overfitting, divergence)?

Hypothesis:
The previous configuration used excessive regularization (weight_decay = 0.8), which likely suppressed gradient updates and slowed convergence.
Reducing weight decay should:
- allow stronger parameter updates
- improve early accuracy
- reduce class collapse behavior

Expected Outcome:
- Faster increase in training and test accuracy
- Lower training loss earlier in training
- Improved per-class balance, especially for weak classes
"""
...
plt.axis("off")
plt.savefig("sample_inputs_ExpOpt_v1.png", dpi=200, bbox_inches="tight")
plt.close()
...
plt.title("Experiment Opt (v1) Confusion Matrix")
plt.tight_layout()
plt.savefig("experimentOptv1_confusion_matrix.png", dpi=200)
plt.close()
...
plt.tight_layout()
plt.savefig("experimentOpt_v1_metrics.png", dpi=200)
plt.close()
