"""
Experiment Optimizer (v3): 3-Qubit QNN Runtime Comparison - Adam Optimizer - 50 Epochs - CPU vs GPU

Objective:
Measure practical runtime differences between CPU and GPU execution for the 3-qubit
Adam-optimized QNN under identical training settings.

Configuration:
- Compute: CPU
- Model: Exp2_v3-style 3-qubit, 8-class classification
- Circuit: 3-layer variational circuit with 2-entanglement layers 
- Observables: Same 8-observable set 
- Sample size: 4000
- Epochs: 50
- Optimizer:
    - Adam
...
Key Questions:
- Does GPU reduce wall-clock training time for this hybrid 3-qubit workflow?
- What is the practical seconds/epoch difference between CPU and GPU?
- Does GPU execution preserve comparable learning behavior over the same short run?
- Is hybrid CPU↔GPU transfer overhead large enough to offset acceleration benefits?

Hypothesis:
Because the hybrid pipeline repeatedly transfers data between PyTorch and CUDA-Q,
GPU acceleration may provide limited or only modest speedup at this scale.
However, with Adam converging quickly, a short-run comparison should reveal whether
GPU execution is practically worthwhile for future 3-qubit experiments.

Expected Outcome:
- CPU and GPU produce similar accuracy over 50 epochs
- GPU may provide limited or moderate speedup
- If overhead dominates, runtime gains may be small despite CUDA availability
"""
...
plt.axis("off")
plt.savefig("sample_inputs_ExpOpt_v3CPU.png", dpi=200, bbox_inches="tight")
plt.close()
...
class QuantumFunction(Function):
    ...
    def __init__(self, qubit_count: int):
        """Define a 3-qubit variational circuit with one entangling gate."""
        ...
        def kernel(qubit_count: int, thetas: np.ndarray):
            ...
            # Layer 2
            rx(thetas[3], qubits[0])
            rx(thetas[4], qubits[1])
            rx(thetas[5], qubits[2])
            ...
            # Layer 3
            ry(thetas[6], qubits[0])
            ry(thetas[7], qubits[1])
            ry(thetas[8], qubits[2])
...
class HybridQNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc5 = nn.Linear(32, 9)   # Nine circuit parameters
    ...
    def forward(self, x):
        ...
        # Quantum layer returns X logits
        x = self.quantum(x)
        return x
...
plt.title("Experiment Opt (v3) Confusion Matrix")
plt.tight_layout()
plt.savefig("experimentOptv3CPU_confusion_matrix.png", dpi=200)
plt.close()
...
plt.tight_layout()
plt.savefig("experimentOpt_v3CPU_metrics.png", dpi=200)
plt.close()
