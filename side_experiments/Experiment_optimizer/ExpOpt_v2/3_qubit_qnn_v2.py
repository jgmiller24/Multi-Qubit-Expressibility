"""
Experiment Optimizer (v2): 3-Qubit QNN Optimization Study - Adam Optimizer - EPOCHS = 200


Objective:
Evaluate whether switching from Adadelta to Adam improves convergence speed, training stability, and early classification performance in the 3-qubit QNN.
...
- Observed moderate convergence (~43% accuracy at 200 epochs)

Key Questions:
- Does Adam improve convergence speed compared to Adadelta?
- Does Adam achieve higher accuracy within the same training window (200 epochs)?
- Are weak classes (collapse cases) improved?
- Does training remain stable (no divergence or overfitting)?

Hypothesis:
Adaptive optimization (Adam) will improve gradient efficiency and accelerate convergence compared to Adadelta, leading to:
- faster accuracy gains
- improved early performance
- better handling of uneven class learning

Expected Outcome:
- Higher accuracy than Adadelta at 200 epochs
- Faster reduction in training and test loss
- Improved per-class balance, especially for previously weak classes
"""
...
plt.axis("off")
plt.savefig("sample_inputs_ExpOpt_v2.png", dpi=200, bbox_inches="tight")
plt.close()
...
plt.title("Experiment Opt (v2) Confusion Matrix")
plt.tight_layout()
plt.savefig("experimentOptv2_confusion_matrix.png", dpi=200)
plt.close()
...
plt.tight_layout()
plt.savefig("experimentOpt_v2_metrics.png", dpi=200)
plt.close()
