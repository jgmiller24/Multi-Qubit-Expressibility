"""
Experiment 2 (v3):  3-Qubit Scaling Evaluation - Extended Training (600 → 1000 epochs)

Objective:
Isolate the effect of training time by holding the circuit fixed at the v2 architecture
and increasing epochs from 600 to 1000. Determines whether v2's ~45–46% accuracy was
training-time limited before adding circuit depth in v4.
...
Circuit Structure (v3 = v2 circuit, unchanged):
  RY layer → entanglement (q0→q1→q2) → RX layer
  Total variational parameters: 6

Key Questions:
- Does extended training improve on v2's ~45–46% test accuracy?
- Has the v2 circuit already converged, or is it still learning at epoch 600?
- What does the loss/accuracy curve shape look like over 1000 epochs?
...
plt.axis("off")
plt.savefig("sample_inputs_Exp2_v3.png", dpi=200, bbox_inches="tight")
plt.close()
...
class QuantumFunction(Function):
    ...
    def __init__(self, qubit_count: int):
        """Define a 3-qubit variational circuit with one entangling block (identical to v2)."""
    ...
    def run(self, theta_vals: torch.Tensor) -> torch.Tensor:
        """Evaluate eight observables to produce an 8D quantum output vector."""
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(thetas.shape[1]):
            ...
            exp_vals_plus = ctx.quantum_circuit.run(thetas_plus)   # (batch, 8)

            thetas_minus = thetas.clone()
            thetas_minus[:, i] -= ctx.shift
            exp_vals_minus = ctx.quantum_circuit.run(thetas_minus) # (batch, 8)
...
class HybridQNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc5 = nn.Linear(32, 6)   # Six circuit parameters (same as v2)
    ...
    def forward(self, x):
        ...
        # Quantum layer returns 8 logits
        x = self.quantum(x)
        return x
...
plt.title("Experiment 2 (v3) Confusion Matrix")
plt.tight_layout()
plt.savefig("experiment2v3_confusion_matrix.png", dpi=200)
plt.close()
...
plt.tight_layout()
plt.savefig("experiment2_v3_metrics.png", dpi=200)
plt.close()
